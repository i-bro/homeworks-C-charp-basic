...
List<Product> priceGreaterThan500 = products.Where(prod => prod.Price > 500).ToList();

//2. Retrive all Skincare products.
List<Product> skincareProd = products.Where(prod => prod.Category == ProductCategory.Skincare).ToList();

//3. Retrive all products titles.
List<string> productTitles = products.Select(prod => prod.Title).ToList();

//4. Select the titles of all products in the "Laptops" category.
List<string> titlesOfLaptops = products
    .Where(prod => prod.Category == ProductCategory.Laptops)
    .Select(prod => prod.Title).ToList();

//5. Select the descriptions of all products with a stock of less than 50.
List<string> descriptionOfStockLess50 = products
    .Where(prod => prod.Stock < 50)
    .Select(prod => prod.Description)
    .ToList();

//6.Retrieve the titles of all products with a rating above 4.5.
List<string> titlesWithHighRating = products
    .Where(prod => prod.Rating > 4.5)
    .Select(prod => prod.Title)
    .ToList();

//7. Select the titles of all products with a price between $100 and $200.
List<string> titlesWithPriveBtvn100And200 = products
    .Where(prod => prod.Price >= 100 && prod.Price  <= 200)
    .Select(prod => prod.Title)
    .ToList();

//9. Retrieve the brands of all products with a price above $1000.
List<string> brandsWithPrice1000 = products
    .Where(prod => prod.Price > 1000)
    .Select(prod => prod.Brand)
    .ToList();

//10. Select the titles of all products with the word "perfume" in the title.
List<string> titlesOfProdPerfume = products
    .Where(prod => prod.Title.Contains("perfume", StringComparison.OrdinalIgnoreCase))
    .Select(prod => prod.Title)
    .ToList();

//11. Find the last Grocery product.
Product lastGrocery = products.LastOrDefault(prod => prod.Category == ProductCategory.Groceries);

//12. Find the first product with a price above $1000
Product firstProdbove1000 = products.FirstOrDefault(prod => prod.Price > 1000);

//13. Select the titles of all products from the "Groceries" category with a stock above 150.
List<string> titlesOfProdFromGroceries = products
    .Where(prod => prod.Category == ProductCategory.Groceries && prod.Stock > 150)
    .Select(prod => prod.Title)
    .ToList();

//14. Find the first item from the brand "Hemani Tea".
Product firstFromBrantHemani = products.FirstOrDefault(prod => prod.Brand == "Hemani Tea");

//Retrieve the ratings of all products with a stock between 30 and 50.
List<double> productsStock30to50 = products
    .Where(prod => prod.Stock >= 30 && prod.Stock <= 50)
    .Select(prod => prod.Rating)
    .ToList();

//16. Find the average price of all products.
double averagePrice = products.Average(prod => prod.Price);

//17. Find the total stock of all products.
int totalStock = products.Sum(prod => prod.Stock);

//18. Check if there is any product with price over $2000.
bool isAnyProdWithPrice2000 = products.Any(prod => prod.Price == 2000);

//19. Find the most expensive Laptop.
Product mostExspensiveLaptop = products
    .Where(prod => prod.Category == ProductCategory.Laptops)
    .OrderByDescending(prod => prod.Price)
    .FirstOrDefault();

//20. Retrieve the titles and descriptions of all products from the "Skincare" category.
var skincareInfo = products
